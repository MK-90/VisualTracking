...
for frame = 1:num_frames,
    ...
    if frame > 1
        % compute the compressed learnt appearance 计算学到的外观特征，相当于模板。  投影矩阵 projection_matrix 是第一主成成分
        zp = feature_projection(z_npca, z_pca, projection_matrix, cos_window);
        
        % extract the feature map of the local image patch 提取两种特征
        [xo_npca, xo_pca] = get_subwindow(im, pos, sz, non_compressed_features, compressed_features, w2c);
        
        % do the dimensionality reduction and windowing 对特征降维和加窗
        x = feature_projection(xo_npca, xo_pca, projection_matrix, cos_window);
        
        % calculate the response of the classifier 计算响应， x和zp的互相关
        kf = fft2(dense_gauss_kernel(sigma, x, zp)); %使用标准差为sigma的高斯核计算多维特征图x和zp的核输出
    ...
    %每一帧计算了pos之后，在该位置提取这一帧图像的 pca 特征和 non-pca 特征，结合之前特征来更新这两种特征
    ...
    % if dimensionality reduction is used: update the projection matrix 如果进行降维，就更新投影矩阵
    if use_dimensionality_reduction
        ...
        % calculate the principal components (pca_basis) and corresponding variances 主成成分矩阵pca_basis
        if frame == 1
        ...
        projection_matrix = pca_basis(:, 1:num_compressed_dim); %这里得到了新的投影矩阵
    ...
    kf = fft2(dense_gauss_kernel(sigma, x)); % %使用标准差为sigma的高斯核计算多维特征图x的核输出，自相关
